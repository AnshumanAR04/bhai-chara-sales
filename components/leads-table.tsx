...
export async function LeadsTable({ searchParams }: LeadsTableProps) {
  ...
  if (searchParams.search) {
    query = query.or(
      `farmer.name.ilike.%${searchParams.search}%,farmer.phone.ilike.%${searchParams.search}%,notes.ilike.%${searchParams.search}%`,
    )
  }
      ...
      <CardContent>
        <div className="rounded-md border">
          <Table>
            ...
            <TableBody>
              {leads?.map((lead) => (
                <TableRow key={lead.id} className={`border-l-4 ${getPriorityColor(lead.status, lead.created_at)}`}>
